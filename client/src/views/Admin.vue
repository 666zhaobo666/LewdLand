<template>
  <div class="space-y-8">
    <section
      v-if="!authed"
      class="mx-auto max-w-md rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900"
    >
    ...
    <div v-else class="space-y-8">
      <section class="flex flex-wrap items-center gap-3">
        <div>
          ...
          <p class="text-sm text-neutral-500">扫描会在后台运行，执行期间会在这里持续显示状态，不影响其他操作。</p>
        </div>
        <div class="ml-auto flex flex-wrap gap-2">
          <button @click="enqueueAllScans" class="rounded-lg border border-neutral-300 px-3 py-1.5 text-sm dark:border-neutral-700">
            刷新全部扫描
          </button>
          <button @click="reloadData" class="rounded-lg border border-neutral-300 px-3 py-1.5 text-sm dark:border-neutral-700">
            刷新页面数据
          </button>
          <button @click="logout" class="rounded-lg border border-neutral-300 px-3 py-1.5 text-sm dark:border-neutral-700">
            退出
          </button>
        </div>
      </section>

      <section v-if="visibleJobs.length" class="rounded-2xl border border-neutral-200 p-4 dark:border-neutral-800">
        <div class="mb-3 flex items-center gap-3">
          <h2 class="text-lg font-bold">扫描状态</h2>
          <span class="text-sm text-neutral-500">
            {{ visibleJobs.length }} 个进行中的任务，并发 {{ maxConcurrent }}
          </span>
          <button @click="clearFinishedJobs" class="ml-auto text-sm text-blue-600 hover:underline">
            清理已完成
          </button>
        </div>
        ...
        <div class="space-y-3">
          ...
          >
            ...
            <div class="mt-3 h-2 overflow-hidden rounded-full bg-neutral-200 dark:bg-neutral-800">
              <div class="h-full bg-blue-600 transition-all duration-300" :style="{ width: `${jobPercent(job)}%` }"></div>
            </div>

            <div
              v-if="job.summary || job.error"
              class="mt-2 whitespace-pre-line text-xs"
              :class="job.error ? 'text-red-500' : 'text-neutral-500'"
            >
      ...
      <section class="rounded-2xl border border-neutral-200 p-4 dark:border-neutral-800">
        <div class="mb-3 flex items-center gap-3">
          ...
          <button @click="openThemeModal()" class="ml-auto rounded-lg bg-blue-600 px-3 py-1.5 text-sm text-white">
            + 新建主题
          </button>
          <button @click="exportConfig(false)" class="rounded-lg border border-neutral-300 px-3 py-1.5 text-sm dark:border-neutral-700">
            导出配置
          </button>
          ...
          >
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span class="text-xs text-neutral-500">
                {{ theme.source_count }} 个数据源 · {{ theme.message_count }} 条资源
              </span>
              <div class="ml-auto flex flex-wrap gap-2">
                <button @click="openThemeModal(theme)" class="rounded-lg border border-neutral-300 px-2.5 py-1 text-xs dark:border-neutral-700">
                  编辑
                </button>
                <button @click="scanTheme(theme)" class="rounded-lg bg-emerald-600 px-2.5 py-1 text-xs text-white">
                  单独扫描
                </button>
                <button @click="deleteTheme(theme)" class="rounded-lg border border-red-300 px-2.5 py-1 text-xs text-red-600 dark:border-red-900">
                  删除
                </button>
              </div>
            </div>
      ...
      <section v-if="selectedTheme" class="rounded-2xl border border-neutral-200 p-4 dark:border-neutral-800">
        <div class="mb-3 flex items-center gap-3">
          <div>
            ...
            <p class="text-sm text-neutral-500">这里可以逐个测试、编辑和扫描数据源。</p>
          </div>
          <button @click="openSourceModal()" class="ml-auto rounded-lg bg-blue-600 px-3 py-1.5 text-sm text-white">
            + 添加数据源
          </button>
        </div>
        ...
        <div v-else class="space-y-2">
          ...
          >
            <div class="flex flex-wrap items-center gap-2">
              ...
              <div class="ml-auto flex flex-wrap gap-2">
                <button @click="testSourceOnList(source)" class="rounded-lg border border-neutral-300 px-2.5 py-1 text-xs dark:border-neutral-700">
                  测试
                </button>
                <button @click="scanSource(source)" class="rounded-lg bg-emerald-600 px-2.5 py-1 text-xs text-white">
                  扫描
                </button>
                <button @click="openSourceModal(source)" class="rounded-lg border border-neutral-300 px-2.5 py-1 text-xs dark:border-neutral-700">
                  编辑
                </button>
                <button @click="deleteSource(source)" class="rounded-lg border border-red-300 px-2.5 py-1 text-xs text-red-600 dark:border-red-900">
                  删除
                </button>
              </div>
            </div>
            <div class="mt-2 break-all text-xs text-neutral-500">{{ source.type === 'local' ? source.local_path : source.webdav_url }}</div>
            <div v-if="source._test" class="mt-2 text-xs" :class="source._test.ok ? 'text-green-600' : 'text-red-500'">
              {{ source._test.message }}
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="rounded-2xl border border-neutral-200 p-4 dark:border-neutral-800">
        ...
        <form class="mt-3 flex max-w-2xl flex-wrap gap-2" @submit.prevent="changePassword">
          <input
            ...
            type="password"
            placeholder="当前密码"
            class="min-w-[180px] flex-1 rounded-lg border border-neutral-300 bg-white px-3 py-2 dark:border-neutral-700 dark:bg-neutral-950"
          />
          <input
            v-model="pwd.next"
            type="password"
...
const visibleJobs = computed(() => scanJobs.value.filter((job) => job.status === 'queued' || job.status === 'running'));
...
function mergeJob(job) {
  const idx = scanJobs.value.findIndex((item) => item.id === job.id);
  if (idx >= 0) {
...
function removeFinishedJobsFromState() {
...
async function refreshJobs() {
  ...
  scanJobs.value = (data.jobs || []).filter((job) => job.status === 'queued' || job.status === 'running');
...
function startJobsListStream() {
  ...
  stopJobsStream = api.scanJobsStream((event) => {
    if (event.type === 'list' && Array.isArray(event.jobs)) {
      scanJobs.value = event.jobs.filter((job) => job.status === 'queued' || job.status === 'running');
    }
  });
}

function stopJobsListStream() {
  if (!stopJobsStream) return;
  stopJobsStream();
  stopJobsStream = null;
}
...
async function reloadData() {
  ...
  if (selectedTheme.value) {
    const current = themes.value.find((theme) => theme.id === selectedTheme.value.id) || selectedTheme.value;
    await selectTheme(current);
  }
  await refreshJobs();
}
...
async function enqueueAllScans() {
...
async function runSourceTest(payload) {
  try {
...
async function testSourceFromModal() {
  ...
  const result = await runSourceTest({
    type: current.type,
...
async function testSourceOnList(source) {
  ...
  source._test = await runSourceTest({
    type: source.type,
...
async function clearFinishedJobs() {
  await api.clearFinishedScanJobs().catch(() => {});
  removeFinishedJobsFromState();
  await refreshJobs();
}
