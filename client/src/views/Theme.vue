<template>
  <div>
    ...
    <div v-if="loading && items.length === 0" class="py-20 text-center text-neutral-400">加载中...</div>
  ...
  if (!route.query.page && !route.query.limit && !route.query.q) {
    ...
    if (raw) {
      try {
        ...
        page.value = parsePositive(saved.page, page.value);
        limit.value = parsePositive(saved.limit, limit.value);
        q.value = typeof saved.q === 'string' ? saved.q : q.value;
      } catch (_) {
...
function syncThemeUrl(focus = null) {
  return router.replace({
    name: 'theme',
    params: { id: themeId.value },
    query: themeQuery(focus)
  });
}
...
async function onCardClick(event, id) {
  ...
  router.push(messageLink(id));
}

async function load({ restore = false } = {}) {
  if (!isThemeRoute.value) return;
  ...
  try {
    ...
    if (page.value > totalPages.value) {
      ...
      await syncThemeUrl(route.query.focus ? Number(route.query.focus) : null);
      return load({ restore });
    }

    jumpInput.value = page.value;
    if (restore) await restorePosition();
  } catch (error) {
    console.error(error);
  } finally {
    loading.value = false;
...
watch(
  () => [route.params.id, route.query.page, route.query.limit, route.query.q],
  () => {
...
onDeactivated(() => {
  const focusId = route.query.focus ? Number(route.query.focus) : null;
  rememberListState(focusId || null);
});

onActivated(() => {
  if (!isThemeRoute.value) return;
  if (pendingRestore.value || route.query.focus) {
